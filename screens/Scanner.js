   ...
   const handleBarCodeRead = async (event) => {
    if (isCameraReady) {
      try {
        ...
        const response = await fetch(`https://umofreight.com/api/v1/auth/shipment_information/service_type?service_type=${scannedData}`, {
          method: 'GET',
          headers: {
        ...
        console.error('Error sending or logging data:', error);
      }
    }
  };  
